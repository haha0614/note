...
![image-20240904114201573](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904114201573.png)

其他面试题

![image-20240904114227426](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904114227426.png)

---

# 使用场景

![image-20240904114423098](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904114423098.png)

## 缓存穿透

![image-20240904114530108](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904114530108.png)
...
![image-20240904114906531](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904114906531.png)

![image-20240904115007646](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904115007646.png)
...
![image-20240904115339614](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904115339614.png)

![image-20240904115454436](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904115454436.png)

**误判率**: 数组越小误判率越大, 数组越大误判率就越小, 但是同时带来了更多的内存消耗

![image-20240904115604020](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904115604020.png)

![image-20240904115852278](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904115852278.png)

## 缓存击穿

**缓存击穿**: 给某一个key设置了过期时间, 当key过期的时候, 恰好这时间点对这个key有大量的并发请求过来, 这些并发的请求可能会瞬间把DB压垮

![image-20240904120815786](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904120815786.png)

**解决方案一**： 互斥锁

![image-20240904121523665](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904121523665.png)

**强一致**、**性能差**

**解决方案二**： 逻辑过期

![image-20240904122104764](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904122104764.png)
...
- Redis服务宕机: 利用Redis集群提高服务的可用性. <font color='red'>哨兵模式、集群模式</font>

- 给缓存业务添加降级限流策略. <font color='red'>Nginx或者Spring Cloud Gateway</font>、<font color='red'>降级可作为系统的保底策略, 适用于穿透、击穿、雪崩</font>

- 给业务添加多级缓存. <font color='red'>Guava或Caffeine</font>
...
![image-20240904124142416](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904124142416.png)
...
![image-20240904124349399](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904124349399.png)

- 读操作: 缓存命中, 直接返回; 缓存为命中查询数据库, 写入缓存, 设定超时时间

- 写操作: <font color='red'>延迟双删</font>

  ![image-20240904124541067](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904124541067.png)

  1. 先删除缓存, 还是先修改数据库?

     ![image-20240904124737847](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904124737847.png)

     ![image-20240904124951174](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904124951174.png)
  ...
  3. 为什么要延时删除?
     ...
     ![image-20240904125143819](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125143819.png)

### 强一致

![image-20240904125312849](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125312849.png)

**优化**: 一般存入缓存的数据都是<font color='red'>读多写少</font>的数据

**共享锁**: 读锁`readLock`, 加锁之后, 其他线程可以共享读操作

**排他锁**: 也叫独占锁`writeLock`, 加锁之后, 阻塞其他县城读写操作

![image-20240904125602507](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125602507.png)

**实现:**

**读锁:** 

![image-20240904125639061](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125639061.png)

**独占锁:**

![image-20240904125701086](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125701086.png)

### 延迟一致

异步通知保证数据的最终一致性

![image-20240904125910586](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904125910586.png)

基于Canal(阿里的一款中间件)的异步通知

![image-20240904130121312](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904130121312.png)
...
- <font color='red'>强一致性的</font>, 采用`Redisson`提供的读写锁
...
![image-20240904162045867](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904162045867.png)
...
`bgsave`开始时会`fork`主进程得到子进程, 子进程<font color='red'>共享</font>主进程的内存数据. 完成`fork`后读取内存数据并写入RDB文件
...
![image-20240904163003575](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904163003575.png)

### AOF

AOF全称为`Append Only File`(追加文件). Redis处理的每一个写命令都会记录在AOF文件, 可以看作是命令日志文件.

![image-20240904163201217](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904163201217.png)
...
![image-20240904163705849](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904163705849.png)
...
<font color='red'>惰性删除、定期删除</font>

### 惰性删除

惰性删除: 设置该key过期时间后, 我们不去管它, 当需要该key时, 我们再检查其是否过期, 如果过期, 我们就删掉它, 反之返回该key

![image-20240904164558107](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904164558107.png)
...
**<font color='red'>惰性删除</font> + <font color='red'>定期删除</font>两种策略进行配合使用**
...
![image-20240904170449326](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904170449326.png)
...
![image-20240904171513406](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171513406.png)

![image-20240904171618364](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171618364.png)

**正常情况:**

![image-20240904171701511](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171701511.png)

**异常情况:**

![image-20240904171821954](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171821954.png)

**解决方案一:** 

![image-20240904171850325](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171850325.png)

![image-20240904171916902](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904171916902.png)

**集群模式下的异常情况:**

![image-20240904172131317](https://gitee.com/w-d1107/typora-image/raw/master/image-20240904172131317.png)

**互斥锁是本地的, 对于集群来说, 没有效果, 此时就需要一个外部的锁, 也就是分布式锁**

![image-20240905094145817](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905094145817.png)
...
- 获取锁:
  ...
  ![image-20240905094359723](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905094359723.png)

- 释放锁:

  ![image-20240905094417002](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905094417002.png)

![image-20240905094625466](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905094625466.png)
...
![image-20240905095304228](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905095304228.png)
...
![image-20240905095621240](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905095621240.png)

<font color='red'>如果设置了锁的过期时间, 将不会触发看门狗机制, 也就不会给锁续期. </font>

<font color='red'>加锁、设置过期时间等操作都是基于`lua`脚本完成</font>

`lua`脚本作用: 能够调用Redis命令, 保证多条命令执行的原子性

### Redisson实现的分布式锁-可重入

![image-20240905100125682](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905100125682.png)

利用<font color='red'>hash结构</font>记录<font color='red'>线程id</font>和<font color='red'>重入次数</font>

![image-20240905100428160](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905100428160.png)

### Redisson实现的分布式锁-主从一致性

![image-20240905100901524](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905100901524.png)

RedLock(红锁): 不能只在一个Redis实例上创建锁, 应该是在多个Redis实例上创建锁<font color='red'>(n / 2 + 1)</font>, 避免在一个Redis实例上加锁

![image-20240905101149441](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905101149441.png)

![image-20240905101228911](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905101228911.png)
...
答: 在Redisson的分布式锁中, 提供了一个`WatchDog`(看门狗), 一个线程获取锁成功以后, `WatchDog`会给持有锁的线程<font color='red'>续期(默认是(release(默认30s) / 3) 10s续期一次)</font>

### 面试官: Redisson的这个锁， 可以重入吗?

答: 可以重入, 多个锁重入需要判断是否是当前线程, 在Redis中进行存储的时候使用的是<font color='red'>hash结构</font>, 来存储<font color='red'>线程信息和重入的次数</font>

### 面试官: Redisson锁能解决主从一致的问题吗

答: 不能解决, 但是可以使用Redisson提供的<font color='red'>红锁</font>来解决, 但是这样的话, <font color='red'>性能就太低了</font>, 如果业务中非要<font color='red'>保证数据的强一致性</font>, 建议采用<font color='red'>zookeeper</font>实现的分布式锁
...
主从<font color='red'>**全量同步**</font>: 

![image-20240905103652193](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905103652193.png)
...
主从<font color='red'>**增量同步**</font>(`slave`重启或后期数据变化)

![image-20240905104156260](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905104156260.png)
...
![image-20240905111136391](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905111136391.png)
...
- 主观下线: 如果某Sentinel节点发现某实例未在规定时间相应, 则认为该实例**主观下线**.
- 客观下线: 若超过指定数量(`quorum`)的Sentinel都任务该实例主观下线, 则该实例**客观下线**. `quorum`值最好超过Sentinel实例数量的一半

![image-20240905111748834](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905111748834.png)

**哨兵选主规则**

- 首先判断主与从节点断开时间长短, 如超过指定值就排除该从节点
- 然后判断从节点的`slave-priority`值, 越小优先级越高
- <font color='red'>如果`slave-prority`一样, 则判断`slave`节点的`offset`值, 越大优先级越高</font>
...
![image-20240905112431914](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905112431914.png)

如果此时发生了网络等一系列原因故障, 导致主从节点不在同一个网络分区下, 如下图:

![image-20240905112512238](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905112512238.png)

此时Sentinel会根据选主规则, 重新选择一个主节点. 问题来了, 此时客户端连接的依旧是之前的主节点, 还会继续往老主节点中写入数据, 而不会往新主节点中写入. 当网络恢复后, 老主节点会被强制降为子节点, 如图:

![image-20240905112703192](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905112703192.png)
...
答: <font color='red'>集群脑裂</font>是由于主节点和从节点和Sentinel处于不同的网络分区, 使得Sentinel没有能够心跳感知到主节点, 所以通过选举的方式提升了一个从节点为主, 这样就存在了两个`master`, 就像大脑分裂了一样, 这样会导致客户端还在老的主节点那里写入数据, 新节点无法同步数据, 当网络恢复后, Sentinel会将老的主节点降为从节点, 这是再从新`master`同步数据, 就会导致数据丢失
...
![image-20240905114451774](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905114451774.png)

#### 数据读写

Redis分片集群引入了哈希槽的概念, Redis集群有16384个哈希槽, 每隔`key`通过`CRC16`校验后对16384取模来决定放置哪个槽, 集群的每个节点负责一部分hash槽

![image-20240905114926692](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905114926692.png)
...
- 将16384个插槽分配到不同的实例
- 读写数据: 根据`key`的<font color='red'>有效部分</font>计算哈希值, 对16384取余(<font color='red'>有效部分</font>, 如果key前面有大括号, 大括号的内容就是有效部分, 如果没有, 则以`key`本身作为有效部分)余数作为插槽, 寻找插槽所在的实例
...
![image-20240905120525257](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905120525257.png)
...
![image-20240905121032257](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905121032257.png)
...
![image-20240905121251008](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905121251008.png)
...
![image-20240905121656350](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905121656350.png)
...
![image-20240905122904129](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905122904129.png)

在Redis6.0版本之后加入了多线程模式

![image-20240905123257518](https://gitee.com/w-d1107/typora-image/raw/master/image-20240905123257518.png)
