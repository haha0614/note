...
![image-20240729132710982](./Redis面试篇/image-20240729132710982.png)
...
| **容器名** | **角色** | **IP**         | **映射端口** |
| :--------- | :------- | :------------- | :----------- |
| r1         | master   | 192.168.11.128 | 7001         |
| r2         | slave    | 192.168.11.128 | 7002         |
| r3         | slave    | 192.168.11.128 | 7003         |

#### 3.1.启动多个Redis实例

我们利用课前资料提供的docker-compose文件来构建主从集群：

![img](./Redis面试篇/1722231184108-1.png)
...
![img](./Redis面试篇/1722231213076-4.png)
...
![img](./Redis面试篇/1722231227930-7.png)

查看docker容器，发现都正常启动了：

![img](./Redis面试篇/1722231227930-8.png)

由于采用的是host模式，我们看不到端口映射。不过能直接在宿主机通过ps命令查看到Redis进程：

![img](./Redis面试篇/1722231227930-9.png)
...
当主从**第一次同步连接**或**断开重连**时, 从节点都会发送`psynv`请求, 尝试数据同步

![image-20240729141007549](./Redis面试篇/image-20240729141007549.png)
...
![img](./Redis面试篇/1722233454497-18.png)
...
![img](./Redis面试篇/1722233454497-19.png)
...
![img](./Redis面试篇/1722233454497-20.png)

再看下`r2`节点执行`replicaof`命令时的日志：

![img](./Redis面试篇/1722233454497-21.png)
...
![img](./Redis面试篇/1722233496260-30.png)
...
![img](./Redis面试篇/1722233513772-33.png)

slave与master的offset之间的差异，就是salve需要增量拷贝的数据了。

随着不断有数据写入，master的offset逐渐变大，slave也不断的拷贝，追赶master的offset：

![img](./Redis面试篇/1722233513772-34.png)

直到数组被填满：

![img](./Redis面试篇/1722233513772-35.png)

此时，如果有新的数据写入，就会覆盖数组中的旧数据。不过，旧的数据只要是绿色的，说明是已经被同步到slave的数据，即便被覆盖了也没什么影响。因为未同步的仅仅是红色部分：

![img](./Redis面试篇/1722233513772-36.png)

但是，如果slave出现网络阻塞，导致master的`offset`远远超过了slave的`offset`： 

![img](./Redis面试篇/1722233513772-37.png)

如果master继续写入新数据，master的`offset`就会覆盖`repl_baklog`中旧的数据，直到将slave现在的`offset`也覆盖：

![img](./Redis面试篇/1722233513772-38.png)
...
- Redis单节点上的内存占用不要太大，减少RDB导致的过多磁盘IO(一般不超过8G)
- 适当提高`repl_baklog`的大小，发现slave宕机时尽快实现故障恢复，尽可能避免全量同步
- 限制一个master上的slave节点数量，如果实在是太多slave，则可以采用`主-从-从`链式结构，减少master压力

`主-从-从`架构图：

![img](./Redis面试篇/1722233531188-51.png)
...
![img](./Redis面试篇/1722233553405-54.png)
...
- **主观下线(sdown)**：如果某sentinel节点发现某Redis节点(实例)未在规定时间响应，则认为该节点(实例)**主观下线**。
- **客观下线(odown)**：若超过指定数量（通过`quorum`设置）的sentinel都认为该节点主观下线，则该节点(实例)**客观下线**。quorum值最好超过Sentinel节点数量的一半，Sentinel节点数量至少3台。

如图：

![img](./Redis面试篇/1722233553405-55.png)
...
![img](./Redis面试篇/1722233700234-60.png)

假如master发生故障，slave1当选。则故障转移的流程如下：

1）`sentinel`给备选的`slave1`节点发送`slaveof no one`命令，让该节点成为`master`

![img](./Redis面试篇/1722233700234-61.png)

2）`sentinel`给所有其它`slave`发送`slaveof 192.168.11.128 7002` 命令，让这些节点成为新`master`，也就是`7002`的`slave`节点，开始从新的`master`上同步数据。

![img](./Redis面试篇/1722233700234-62.png)

3）最后，当故障节点恢复后会接收到哨兵信号，执行`slaveof 192.168.11.128 7002`命令，成为`slave`：

![img](./Redis面试篇/1722233700234-63.png)
...
![img](./Redis面试篇/1722233727287-72.png)

其内容如下：

```Bash
sentinel announce-ip "192.168.11.128" 
sentinel monitor hmaster 192.168.11.128 7001 2 
...
![img](./Redis面试篇/1722233727287-73.png)
...
services:
  ...
  r2:
    ...
    container_name: r2
    network_mode: "host"
    entrypoint: ["redis-server", "--port", "7002", "--slaveof", "192.168.11.128", "7001"]
  r3:
    image: redis
    container_name: r3
    network_mode: "host"
    entrypoint: ["redis-server", "--port", "7003", "--slaveof", "192.168.11.128", "7001"]
  s1:
    image: redis
    container_name: s1
    volumes:
...
![img](./Redis面试篇/1722233727287-74.png)
...
![img](./Redis面试篇/1722233727287-75.png)
...
![img](./Redis面试篇/1722235536121-84.jpeg)
...
![img](./Redis面试篇/1722235536121-85.jpeg)
...
| ------ | ------ | -------------- | -------- |
| r1     | master | 192.168.11.128 | 7001     |
| r2     | master | 192.168.11.128 | 7002     |
| r3     | master | 192.168.11.128 | 7003     |
| r4     | slave  | 192.168.11.128 | 7004     |
| r5     | slave  | 192.168.11.128 | 7005     |
| r6     | slave  | 192.168.11.128 | 7006     |
...
192.168.11.128:7001 192.168.11.128:7002 192.168.11.128:7003 \
192.168.11.128:7004 192.168.11.128:7005 192.168.11.128:7006
```
...
![image-20240729164047707](./Redis面试篇/image-20240729164047707.png)

这里展示了集群中`master`与`slave`节点分配情况，并询问你是否同意。节点信息如下：

- `7001`是`master`，节点`id`后6位是`774451`
- `7002`是`master`，节点`id`后6位是`0b1aef`
- `7003`是`master`，节点`id`后6位是`787ba5`
- `7004`是`slave`，节点`id`后6位是`8c5756`，认`0b1aef`（7002）为`master`
- `7005`是`slave`，节点`id`后6位是`e152cd`，认`787ba5`（7003）为`master`
- `7006`是`slave`，节点`id`后6位是`4a018a`，认`774451`（7001）为`master`

输入`yes`然后回车。会发现集群开始创建，并输出下列信息：

![image-20240729163712250](./Redis面试篇/image-20240729163712250.png)
...
![image-20240729163737912](./Redis面试篇/image-20240729163737912.png)
...
![image-20240729164849016](./Redis面试篇/image-20240729164849016.png)
...
![image-20240729165243573](./Redis面试篇/image-20240729165243573.png)
...
![image-20240729165328595](./Redis面试篇/image-20240729165328595.png)
...
![image-20240729170018679](./Redis面试篇/image-20240729170018679.png)

可以看到`user:{age}`和`age`计算出的`slot`都是`741`。

> - [x] **Radis如何判断某个key应该在哪个实例**
>
>   
>
>   - 将16384个插槽分配到不同的实例
...
![img](./Redis面试篇/1722235551558-111.png)

过了一段时间后，7002原本的小弟7006变成了`master`：

![img](./Redis面试篇/1722235551558-112.png)
...
在Redis分片集群中, 会有16384个插槽, Redis会把这些插槽分配给分片集群中的每一个master节点. 当我们根据一个key进行数据读写的时候, 我们就可以根据key做hash运算, 得到一个结果, 再根据结果与16384取余, 得到的值就是当前key的插槽值, 看一下插槽值在哪个节点上, 就把数据存到哪个节点, 或者从某个节点去读
...
![img](./Redis面试篇/1722235551559-113.png)
...
| 5        | `OBJ_ENCODING_ZIPLIST`    | 压缩列表(内存占用小)             |
| 6        | `OBJ_ENCODING_INTSET`     | 整数集合                         |
| 7        | `OBJ_ENCODING_SKIPLIST`   | 跳表                             |
| 8        | `OBJ_ENCODING_EMBSTR`     | embstr编码的动态字符串           |
| 9        | `OBJ_ENCODING_QUICKLIST`  | 快速列表                         |
| 10       | `OBJ_ENCODING_STREAM`     | Stream流                         |
| 11       | `OBJ_ENCODING_LISTPACK`   | 紧凑列表(内存相对小, 性能好一点) |
...
| SET          | `intset`(数据量较小时, 紧凑结构, 内存小, 连续空间, 读写效率高)、`HT`(数据量较大时使用) |
...
https://b11et3un53m.feishu.cn/wiki/Jck7w4GBSia4sukQn1vc9s3anMf#WQeJdt7doo1Tywx4DF0c0acgnJD
...
![img](./Redis面试篇/1722235551559-114.png)

我们可以看到1号元素就有指向3、5、10的多个指针，查询时就可以跳跃查找。例如我们要找大小为14的元素，查找的流程是这样的：

![img](./Redis面试篇/1722235551559-115.png)
...
![img](./Redis面试篇/1722235551559-116.png)
...
- 每组数据都包含score和element/member

- element/member唯一

- 可根据score排序

  ![image-20240730090310316](./Redis面试篇/image-20240730090310316.png)

> - [x] **面试题**：Redis的`SortedSet`底层的数据结构是怎样的？
>
>   **答**：SortedSet是有序集合，底层的存储的每个数据都包含element/member和score两个值。score是得分，element/member则是字符串值。SortedSet会根据每个element的score值排序，形成有序集合。
>
>   它支持的操作很多，比如：
>
>   - 根据element/member查询score值
>   - 按照score值升序或降序查询element/member
>
>   要实现根据element查询对应的score值，就必须实现element/member与score之间的键值映射。SortedSet底层是基于**HashTable**来实现的。
>
...
>   **或答:**
>   
>   - 首先SortedSet需要能存储score和member值, 而且要快捷的根据member查询score, 因此底层有一个哈希表, 以member为键, 以score为值
...
![img](./Redis面试篇/1722235566664-129.png)
...
- `volatile-lru`： 对设置了TTL的key，基于LRU算法进行淘汰(**其次**)
- `allkeys-lfu`： 对全体key，基于LFU算法进行淘汰
- `volatile-lfu`： 对设置了TTL的key，基于LFI算法进行淘汰(**推荐**)
...
![img](./Redis面试篇/1722235566664-130.png)
...
![img](./Redis面试篇/1722235566664-131.png)
...
- `Write Behind Cahing`：读写操作都直接操作缓存，由线程异步的将缓存数据同步到数据库(**高性能, 弱一致性/最终一致性**)
...
![img](./Redis面试篇/1722235596246-138.png)

**异常情况**

![img](./Redis面试篇/1722235596246-139.png)
...
![img](./Redis面试篇/1722235596246-140.png)

**异常情况**

![img](./Redis面试篇/1722235596246-141.png)
...
> - [x] **综上**，添加缓存的目的是为了提高系统性能，而你要付出的代价就是缓存与数据库的强一致性。如果你要求数据库与缓存的强一致，那就需要加锁避免并行读写。但这就降低了性能，与缓存的目标背道而驰。
>
...
![img](./Redis面试篇/1722235596246-142.png)
...
![img](./Redis面试篇/1722235596247-143.png)
...
![img](./Redis面试篇/1722235596247-144.png)
...
![img](./Redis面试篇/1722235596247-145.png)

### 3.缓存雪崩

缓存雪崩是指在同一时段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力。

![img](./Redis面试篇/1722235596247-146.png)
...
![img](./Redis面试篇/1722235596247-147.png)
...
![img](./Redis面试篇/1722235596247-148.png)

逻辑过期的思路如图：

![img](./Redis面试篇/1722235596247-149.png)

![image-20240730111425500](./Redis面试篇/image-20240730111425500.png)
