...
![image-20240730124602406](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730124602406.png)

当我们修改其中一个节点的数据时，两者的数据产生了差异：

![image-20240730124615544](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730124615544.png)

要想保住一致性，就必须实现node01 到 node02的数据 同步：

![image-20240730124627149](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730124627149.png)
...
![image-20240730124815351](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730124815351.png)
...
![image-20240730124843236](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730124843236.png)
...
- **`S`**`oft State`**(软状态)**: 在一定时间内，允许出现中间状态，比如**临时**的不一致状态。
...
![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639268-10.png)
...
![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-1.png)

这种模式在大多数情况下（99%）并不会有什么问题，不过在极端情况下，特别是多线程并发访问AT模式的分布式事务时，有可能出现脏写问题，如图：

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-2.png)

解决思路就是引入了全局锁的概念。在释放DB锁之前，先拿到全局锁。避免同一时刻有另外一个事务来操作当前数据。

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-3.png)

由于全局锁由TC提供, 所以只能限制由seata管理的事务, 如果事务2不由seata管理, 还是会出现脏写问题

这种问题不用考虑, 原因有三:

1. 事务回滚概率小
2. 见缝插针概率更小
3. 开发人员在开发时要避免出现多事务不由seata管理并操作同一份数据

虽然不太可能出现, 但也要避免

seata的解决方法如下:

![image-20240730132807418](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730132807418.png)
...
![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-4.png)

余额充足，可以冻结：

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-5.png)

此时，总金额 = 冻结金额 + 可用金额，数量依然是100不变。事务直接提交无需等待其它事务。

**阶段二（Confirm)**：假如要提交（Confirm），之前可用金额已经扣减，并转移到冻结金额。因此可用金额不变，直接冻结金额扣减30即可：

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-6.png)

此时，总金额 = 冻结金额 + 可用金额 = 0 + 70  = 70元

**阶段二(Canncel)**：如果要回滚（Cancel），则释放之前冻结的金额，也就是冻结金额扣减30，可用余额增加30

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-7.png)

TCC的工作模型图:

![image-20240730145705128](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730145705128.png)
...
![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-8.png)

如果阻塞时间太长，可能导致全局事务超时而触发二阶段的`cancel`操作。两个分支事务都会执行cancel操作：

![img](https://gitee.com/w-d1107/typora-image/raw/master/1722312639266-9.png)
...
![image-20240730150403366](https://gitee.com/w-d1107/typora-image/raw/master/image-20240730150403366.png)

可以去掉消息中间件, 服务2直接暴露业务接口, 服务1在执行完自己的业务后调用服务2, 服务2返回一个成功或失败的结果, 由服务1决定是否重新调接口
...
<img src="./%E5%BE%AE%E6%9C%8D%E5%8A%A1%E9%9D%A2%E8%AF%95%E7%AF%87/1722312670992-31.png" alt="img1651461" />
...
![img](./%E5%BE%AE%E6%9C%8D%E5%8A%A1%E9%9D%A2%E8%AF%95%E7%AF%87/1722312670992-32.png)

默认所有的服务和配置都属于这个`namespace`，当然我们也可以自己创建新的`namespace`：

![img](./%E5%BE%AE%E6%9C%8D%E5%8A%A1%E9%9D%A2%E8%AF%95%E7%AF%87/1722312670992-33.png)

然后填写表单：

![img](./%E5%BE%AE%E6%9C%8D%E5%8A%A1%E9%9D%A2%E8%AF%95%E7%AF%87/1722312670992-34.png)

添加完成后，可以在页面看到我们新建的`namespace`，并且Nacos为我们自动生成了一个命名空间id：

![img](./微服务面试篇/1722312670992-35.png)

我们切换到配置列表页，你会发现`dev`这个命名空间下没有任何配置：

![img](./微服务面试篇/1722312670992-36.png)

因为之前我们添加的所有配置都在`public`下：

![img](./微服务面试篇/1722312670992-37.png)
...
![img](./微服务面试篇/1722312670993-38.png)

而其它服务则出现在`public`下：

![img](./微服务面试篇/1722312670993-39.png)

此时访问`http://localhost:8082/doc.html`，基于`swagger`做测试：

![img](./微服务面试篇/1722312670993-40.png)

会发现查询结果中缺少商品的最新价格信息。

我们查看服务运行日志：

![img](./微服务面试篇/1722312670993-41.png)
...
![img](./微服务面试篇/1722312670993-42.png)
...
![img](./微服务面试篇/1722312670993-43.png)

查看nacos控制台，会发现默认情况下所有服务的集群都是default：

![img](./微服务面试篇/1722312670993-44.png)
...
![img](./微服务面试篇/1722312670993-45.png)

再次查看nacos：

![img](./微服务面试篇/1722312670993-46.png)
...
![img](./微服务面试篇/1722312689124-79.png)

我们可以用idea打开查看一下：

![img](./微服务面试篇/1722312689125-80.png)
...
![img](./微服务面试篇/1722312689125-81.png)
...
但是自SpringCloud2020版本开始，已经弃用Ribbon，改用Spring自己开源的Spring Cloud LoadBalancer了，我们使用的OpenFeign和Gateway也已经与其整合。
...
![img](./微服务面试篇/1722312716926-88.png)
...
![img](./微服务面试篇/1722312716927-100.png)

默认的实现是`RoundRobinLoadBalancer`，即**轮询**负载均衡器。负载均衡器的核心逻辑如下：

![img](./微服务面试篇/1722312716927-101.png)
...
![img](./微服务面试篇/1722312716927-102.png)
...
![img](./微服务面试篇/1722312716927-103.png)
...
![image-20240730124939814](./微服务面试篇/image-20240730124939814.png)

## 3.2.NacosRule

之前分析源码的时候我们发现负载均衡的算法是有`ReactiveLoadBalancer`来定义的，我们发现它的实现类有三个：

![img](./微服务面试篇/1722312752765-136.png)
...
![img](./微服务面试篇/1722312752765-140.png)

DEBUG重启后测试，会发现负载均衡器的类型确实切换成功：

![img](./微服务面试篇/1722312752765-141.png)
...
![img](./微服务面试篇/1722312752765-142.png)
...
![img](./微服务面试篇/1722312752765-143.png)
...
![img](./微服务面试篇/1722312752765-144.png)
...
![img](./微服务面试篇/1722312752766-145.png)

可以看到原本是3个实例，经过筛选后还剩下2个实例。

查看Debug控制台：

![img](./微服务面试篇/1722312752766-146.png)
...
![img](./微服务面试篇/1722312752766-147.png)

那么问题来了， 这个权重是怎么配的呢？

我们打开nacos控制台，进入`item-service`的服务详情页，可以看到每个实例后面都有一个**编辑**按钮：

![img](./微服务面试篇/1722312752766-148.png)

点击，可以看到一个编辑表单：

![img](./微服务面试篇/1722312752766-149.png)

我们将这里的权重修改为5：

![img](./微服务面试篇/1722312752766-150.png)
...
![img](./微服务面试篇/1722312780503-181.jpeg)

两者的优缺点如下：

![img](./微服务面试篇/1722312780503-182.png)

Sentinel的线程隔离就是基于信号量隔离实现的，而Hystix两种都支持，但默认是基于线程池隔离。

> - [x] Sentinel的线程隔离与Hystix的线程隔离有什么区别?
>
>   - 问题说明: 考察对线程隔离方案的掌握情况
>
>   - 难易程度: 一般
...
![img](./微服务面试篇/1722312780503-183.png)
...
![img](./微服务面试篇/1722312780503-184.png)
...
![img](./微服务面试篇/1722312780503-185.png)
...
![img](./微服务面试篇/1722312780503-186.png)
...
![img](./微服务面试篇/1722312780503-189.png)
...
![img](./微服务面试篇/1722312780503-190.png)
...
![img](./微服务面试篇/1722312780504-191.png)

sentinel中的限流中的排队等待功能正是基于漏桶算法实现的。

桶的**容量**取决限流的**QPS阈值**以及允许等待的**最大超时时间**

![image-20240731061733290](./微服务面试篇/image-20240731061733290.png)
...
![image-20240731062332179](./微服务面试篇/image-20240731062332179.png)

![image-20240731062344900](./微服务面试篇/image-20240731062344900.png)

## Sentinel限流与Gateway限流有什么差别
...
>
>   4. Nacos集群默认采用`AP`方式,但也支持`CP`;Eureka采用`AP`方式          
>
> - **Nacos的分级存储模型是什么意思？**
>
...
> - **OpenFeign是如何实现负载均衡的？**
>
>   - Spring在整合OpenFeign的时候，实现`org.springframework.cloud.openfeign.loadbalancer.FeignBlockingLoadBalancerClient`类，其中定义了OpenFeign发起远程调用的核心流程。也就是四步：
>
>     1. 获取请求中的`serviceId`
>
>     2. 根据`serviceId`负载均衡，找出一个可用的服务实例
>
>     3. 利用服务实例的`ip`和`port`信息重构`url`
>
>     4. 向真正的`url`发起请求
>
...
>     - `ReactiveLoadBalancer`：负载均衡器，负责具体的负载均衡算法
>
>   - OpenFeign的负载均衡是基于`Spring-Cloud-Common`模块中的负载均衡规则接口，并没有写死具体实现。这就意味着以后还可以拓展其它各种负载均衡的实现。不过目前SpringCloud中只有`Spring-Cloud-Loadbalancer`这一种实现。
>
...
>
>       - 窗口时间跨度`Interval`为1秒;区间数量n=2,则每个小区间时间跨度为500ms,每个区间都有计数器
>
>       - 限流阈值依然为3,时间窗口(1秒)内请求超过阈值时,超出的请求被限流
>
>       - 窗口会根据当前请求所在时间(`currentTime`)移动,窗口范围是从(`currentTime-Interval`)之后的第一个时区开始,到`currentTime`
>
...
>
>   - 他说,分布式系统无法同时满足这三个指标, 这个结论就叫做`CAP`定理
>
...
>
>     - `AP`模式: 各个事务分别执行和提交,允许出现结果不一致,然后采用弥补措施恢复数据即可,实现最终一致.
>
> - **`AT`模式如何解决脏读和脏写问题的？**
>
>   - `AT`模式也分为两个阶段
...
>       - 如果有任意分支事务失败，则需要根据快照恢复到更新前数据。然后删除快照
>
>   - 这种模式在大多数情况下（99%）并不会有什么问题，不过在极端情况下，特别是多线程并发访问`AT`模式的分布式事务时，有可能出现脏写问题
>
>   - 解决思路就是引入了全局锁的概念。在释放DB锁之前，先拿到全局锁。避免同一时刻有另外一个事务来操作当前数据。
>
...
>
>   5. 当消费者出现异常后，消息会不断`requeue`（重入队）到队列，再重新发送给消费者。如果消费者再次执行依然出错，消息会再次`requeue`到队列，再次投递，直到消息处理成功为止.
>
>      - 失败重试机制：在消费者出现异常时利用本地重试，而不是无限制的`requeue`到mq队列。
>
...
>
>   - 为了解决这个问题，从RabbitMQ的3.6.0版本开始，就增加了`Lazy Queues`的模式，也就是惰性队列。惰性队列的特征如下：
>
>     - 接收到消息后直接存入磁盘而非内存
>
